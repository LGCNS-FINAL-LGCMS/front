...
>("lecturerPagination/fetch", async (params, { rejectWithValue }) => {
  try {
    const response = await apiClient.get(API_ENDPOINTS.LECTURER.GET_LECTURE, {
      params,
    });
...
const lecturePaginationSlice = createSlice({
  name: "lecturer_LectureData",
  initialState,
  reducers: {
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchLecturePage.pending, (state) => {
        state.status = "loading";
        state.error = null;
      })
      .addCase(
        fetchLecturePage.fulfilled,
        (
          state,
      ...
      .addCase(fetchLecturePage.rejected, (state, action) => {
        state.status = "failed";
...
export const { resetPaginationState, setCurrentPage } =
  lecturePaginationSlice.actions;

export default lecturePaginationSlice.reducer;
