    ...
    return (
        <Wrap>
            ...
            <T.Board>
                <T.Table role="table" aria-label="장바구니 강의 목록">
                    ...
                    <tbody>
                        {items.length === 0 ? (
                            <T.TrEmpty>
                                <T.Td colSpan={4} align="center">장바구니가 비어 있습니다.</T.Td>
                            </T.TrEmpty>
                        ) : (
                            items.map(it => (
                                <T.TrBody key={it.lectureId}>
                                    ...
                                    <T.Td align="right">
                                        ...
                                        <RemoveLink type="button" onClick={() => removeOne(it.id)}>🗑</RemoveLink>
            ...
            <BottomBar>
                ...
                <Subtotal>
                    소계 ( 강의 {(selectedCount > 0 ? selectedCount : 0).toLocaleString()} 개):{' '}
...
`;
const LeftGroup = styled.div` display: flex; align-items: center; gap: 10px; `;
const CountText = styled.div` font-weight: 700; `;
...
const Thumb = styled.img`
  width: 40px; height: 40px; border-radius: 8px; object-fit: cover; border: 1px solid ${({ theme }) => theme.colors.border_Light};
`;
const ThumbPlaceholder = styled.div`
  width: 40px; height: 40px; border-radius: 8px; background: ${({ theme }) => theme.colors.background_B}; border: 1px solid ${({ theme }) => theme.colors.border_Light};
`;
const Title = styled.div` font-weight: 800; color: ${({ theme }) => theme.colors.background_D}; margin-bottom: 4px; `;
const Price = styled.div` font-weight: 800; color: ${({ theme }) => theme.colors.background_D}; margin-bottom: 6px; `;
...
const Subtotal = styled.div`
    font-weight: ${({ theme }) => theme.fontSize.subtitle}; color: ${({ theme }) => theme.colors.text_D};
`;